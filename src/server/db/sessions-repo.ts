  ...
  getDb()
    .prepare(
      `INSERT INTO sessions
        (id, source_portrait_id, external_owner_id, favorite_generation_id, status, created_at, updated_at, expires_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      row.id,
...
export function updateSession(id: string, patch: Partial<Omit<SessionRow, "id">>): SessionRow {
  ...
  getDb()
    .prepare(
      `UPDATE sessions
       SET source_portrait_id = ?, external_owner_id = ?, favorite_generation_id = ?, status = ?, updated_at = ?, expires_at = ?
       WHERE id = ?`,
    )
    .run(
